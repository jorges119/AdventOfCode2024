...
object Support:
  ...
  var session = sys.env("AOC_SESSION")
  ...
  inline def Day[N <: Int](day: N)(implementation: Solution[N]): Unit =
    implementation(getInput(day), new Part(day))

  private def getInput(day: Int) =
    val request =
      basicRequest.get(uri"https://adventofcode.com/2024/day/$day/input").header("cookie", s"session=${session}")
  ...
  def getInfiInput(code: String) =
    val request =
      basicRequest.get(uri"https://aoc.infi.nl/api/aoc/input/2024/$code")
    val backend = HttpURLConnectionBackend()
    val response = request.send(backend)
    response.body match {
      case Right(body) => body
      case _           => throw new Exception(s"Error obtaining input for code $code")
    }
